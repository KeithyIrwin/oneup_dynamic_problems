...
function levelOrder(node)
	...
	if next.left ~= None then
	       table.insert(queue,next.left)
	       end
	if next.right ~= None then
	       table.insert(queue,next.right)
	       end
	table.insert( levelOrderAnswer, next.value)
    end
end
...
js = js .. '<div id="graph"></div><script>x=document.getElementById("graph");x.innerHTML = Viz(\''..graphString..'\');</script>';
levelOrder(tree[1])
